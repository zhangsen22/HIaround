...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
		 ...
		 */
			...
			if ( mLastTotalItemCount < totalItemCount )
			{
				canAutoLoad = true;
			}
	...
	public final void onScrollStateChanged(final AbsListView view, final int state) {
		...
		if (state == OnScrollListener.SCROLL_STATE_IDLE && mLastItemVisible) {
			OnRefreshListener2< T > mRefreshListener2 = getOnRefreshListener2( );
			OnRefreshListener< T > mOnRefreshListener = getOnRefreshListener( );
			if ( null != mOnLastItemVisibleListener )
			{
				mOnLastItemVisibleListener.onLastItemVisible();
			}			
			/**
			 * 由于之前都是用OnRefreshListener或者OnRefreshListener2上拉请求数据，6.1要改成自动加载
			 * 所以统一在滑动最后一项可见时在调用onRefresh或者onPullUpToRefresh
			*/			
			Log.d(LOG_TAG, "onScrollStateChanged canAutoLoad***" + canAutoLoad);
			if ( canAutoLoad )
			{
				if ( mRefreshListener2 != null )
				{
					mRefreshListener2.onPullUpToRefresh( this );
				}	
				...
				canAutoLoad = false;
				mLastTotalItemCount = mRefreshableView.getCount( );
			}
		}
	...
	public void setScrollEmptyView(boolean doScroll) {
